provider "google" {
  credentials = file("/Users/anandy/.gcp/credentials/terraform.json")

  project = "snappy-mission-381604"
  region  = "us-central1"
  zone    = "us-central1-c"
}
...
resource "google_service_account" "cloud_run_invoker_sa1" {
  account_id = "cloud-run-invoker-2"
...
resource "google_project_iam_binding" "run_invoker_binding" {
  ...
  members = ["serviceAccount:${google_service_account.cloud_run_invoker_sa1.email}"]
}
...
resource "google_cloud_run_v2_job" "default" {
  ...
  template {
    template {
      containers {
        image = "us-central1-docker.pkg.dev/snappy-mission-381604/quickstart-docker-repo/quickstart-image:tag1"
      }
    }
  }
...
resource "google_cloud_scheduler_job" "job" {
  ...
  http_target {
    ...
    oauth_token {
      service_account_email = google_service_account.cloud_run_invoker_sa1.email
    }
  }
